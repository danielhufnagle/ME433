...
# Change 'COM3' (Windows) or '/dev/ttyUSB0' (Mac/Linux) to match your device's port
SERIAL_PORT = '/dev/tty.usbmodem101' 
BAUD_RATE = 9600

# Initialize serial connection with a timeout so it doesn't freeze the game
try:
...
# Extracted key handling logic into a reusable function for handling inputs
def handle_game_input(input_type):
...
def on_key_down(key):
    # Keyboard controls still work alongside serial input
    if key == keys.UP:
        handle_game_input("up")
    elif key == keys.DOWN:
        handle_game_input("down")
    elif key == keys.RETURN:
        handle_game_input("select")
...
def update(dt):
    ...
    # Check for Serial Data on every frame
    if ser and ser.in_waiting > 0:
        try:
            # Read 1 byte and decode it to a text string
            ...
            pass # Ignore corrupted serial transmissions
